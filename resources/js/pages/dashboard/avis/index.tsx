...
interface Avis {
    ...
    note_globale: number; note_accessibilite: number; note_equipements: number;
    commentaire: string; date: string; verifie: boolean; reponse_partenaire: string | null;
}
interface Stats {
    total: number; note_moyenne: number; taux_5_etoiles: number;
    en_attente_moderation: number; avis_ce_mois: number;
}
...
export default function AvisIndex({
    avis  = [],
    stats = { total:0, note_moyenne:0, taux_5_etoiles:0, en_attente_moderation:0, avis_ce_mois:0 },
}: { avis?: Avis[]; stats?: Stats }) {

    const handleDelete = (id: number) => {
        if (confirm('Masquer cet avis ?')) {
            router.delete(`/avis/${id}`);
        }
    };
                ...
                {avis.length === 0 && (
                    <div className="bg-white rounded-2xl border border-slate-100 flex flex-col items-center justify-center py-16 text-center">
                        <div className="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center mb-3">
                            <svg className="w-6 h-6 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
                        </div>
                        <p className="text-sm font-semibold text-slate-500">Aucun avis pour l'instant</p>
                    </div>
                )}
            </div>
        </DashboardLayout>
