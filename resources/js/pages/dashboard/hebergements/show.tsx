...
const NIVEAU_STYLE: Record<string, { badge: string; bar: string }> = {
    'Excellence':      { badge: 'bg-emerald-100 text-emerald-700 border-emerald-200', bar: 'bg-emerald-500' },
    'Accessible Plus': { badge: 'bg-sky-100 text-sky-700 border-sky-200',             bar: 'bg-sky-500' },
    'Accessible':      { badge: 'bg-teal-100 text-teal-700 border-teal-200',           bar: 'bg-teal-500' },
};
...
export default function HebergementShow({ hebergement, equipements = [], reservations_recentes = [] }: Partial<Props>) {
    const h = hebergement ?? { id: 1, nom: 'Hôtel Azur Accessible', ville: 'Dakar', pays: 'SN', niveau_certif: 'Excellence', note: 4.8, reservations: 24, actif: true, created_at: '12/01/2026' };
    const niveau = NIVEAU_STYLE[h.niveau_certif ?? ''];
                        ...
                        <div className="p-6">
                            ...
                            {h.niveau_certif ? (
                                ...
                                <div className="flex items-center gap-3 p-3 bg-amber-50 border border-amber-100 rounded-xl">
                                    ...
                                    <p className="text-xs text-amber-700 font-medium">Cet établissement n'est pas encore certifié. Un audit est nécessaire.</p>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-100 p-5">
                        ...
                        <div className="space-y-2">
                            ...
                            <button className={`w-full flex items-center gap-3 px-4 py-2.5 border rounded-xl text-sm font-medium transition-all ${h.actif ? 'border-red-200 hover:bg-red-50 text-red-500 hover:border-red-300' : 'border-emerald-200 hover:bg-emerald-50 text-emerald-600 hover:border-emerald-300'}`}>
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={h.actif ? "M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" : "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"}/></svg>
                    ...
                    <div className="bg-red-50 rounded-2xl border border-red-100 p-5">
                        <p className="text-xs font-bold text-red-600 uppercase tracking-wider mb-3">Zone dangereuse</p>
                        <p className="text-xs text-red-500 mb-3">La suppression est irréversible. Toutes les données associées seront perdues.</p>
                        <button className="w-full px-4 py-2.5 bg-red-100 hover:bg-red-200 text-red-600 border border-red-200 rounded-xl text-sm font-semibold transition-colors">
