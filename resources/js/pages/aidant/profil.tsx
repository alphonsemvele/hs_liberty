...
const inputCls = 'w-full px-4 py-3 text-sm border-2 border-slate-200 rounded-xl bg-slate-50 text-slate-900 focus:outline-none focus:border-teal-400 focus:bg-white transition-all';

export default function AidantProfil({ profil }: { profil: any }) {
    ...
    return (
        <AidantLayout title="Mon profil" subtitle="Gérez vos informations">
            ...
            <div className="max-w-2xl space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-slate-100 p-6 flex items-center gap-5">
                    <div className="w-20 h-20 rounded-2xl bg-teal-700 flex items-center justify-center text-3xl font-black text-white flex-shrink-0">
                        {initials}
                    </div>
                    <div>
                        ...
                        <span className="inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-full bg-teal-100 text-teal-700 mt-2">
                            🤝 Aidant
                        </span>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
                    ...
                    <div className="p-6 space-y-4">
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Spécialité / Fonction</label>
                            <input value={data.specialite} onChange={e => setData('specialite', e.target.value)} className={inputCls} placeholder="ex: Aide-soignant, Famille, Bénévole…"/>
                        </div>
                    </div>
                </div>

                <div className="flex justify-end">
                    <button onClick={() => patch('/aidant/profil')} disabled={processing}
                        className="flex items-center gap-2 px-6 py-3 bg-teal-600 hover:bg-teal-700 text-white text-sm font-bold rounded-xl transition-colors disabled:opacity-50">
