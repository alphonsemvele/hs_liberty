...
const inputCls = 'w-full px-4 py-3 text-sm border-2 border-slate-200 rounded-xl bg-slate-50 text-slate-900 focus:outline-none focus:border-violet-400 focus:bg-white transition-all';

export default function TransporteurProfil({ profil }: { profil: any }) {
    ...
    return (
        <TransporteurLayout title="Mon profil" subtitle="Gérez vos informations">
            {flash?.success && (
            ...
            <div className="max-w-2xl space-y-6">
                <div className="bg-white rounded-2xl border border-slate-100 p-6 flex items-center gap-5">
                    <div className="w-20 h-20 rounded-2xl bg-violet-600 flex items-center justify-center text-3xl font-black text-white flex-shrink-0">{initials}</div>
                    <div>
                        ...
                        <span className="inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-full bg-violet-100 text-violet-700 mt-2">🚐 Transporteur PMR</span>
                    </div>
                </div>
                ...
                <div className="flex justify-end">
                    <button onClick={() => patch('/transporteur/profil')} disabled={processing}
                        className="flex items-center gap-2 px-6 py-3 bg-violet-600 hover:bg-violet-700 text-white text-sm font-bold rounded-xl transition-colors disabled:opacity-50">
