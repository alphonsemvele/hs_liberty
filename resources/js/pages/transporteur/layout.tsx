...
export default function TransporteurLayout({ children, title, subtitle }: Props) {
    const { url, props } = usePage<any>();
    const [sidebarOpen, setSidebarOpen] = useState(false);

    const user     = props.auth?.user;
    const prenom   = user?.prenom ?? user?.name?.split(' ')[0] ?? 'Transporteur';
    const initials = ((user?.prenom?.[0] ?? '?') + (user?.nom?.[0] ?? '')).toUpperCase();

    const nav = [
        { href: '/transporteur',         label: 'Tableau de bord', icon: <HomeIcon />,    color: 'text-violet-600 bg-violet-50'  },
        { href: '/transporteur/courses', label: 'Mes courses',     icon: <TruckIcon />,   color: 'text-sky-600 bg-sky-50'        },
        { href: '/transporteur/profil',  label: 'Mon profil',      icon: <UserIcon />,    color: 'text-amber-600 bg-amber-50'    },
    ];

    const isActive = (path: string) => {
        const cur = url.split('?')[0].replace(/\/$/, '');
        const cln = path.replace(/\/$/, '');
        return cur === cln || (cln !== '/transporteur' && cur.startsWith(cln));
    };
        ...
        <>
            <div className="flex items-center gap-3 px-5 h-16 border-b border-slate-100 flex-shrink-0">
                ...
                <div>
                    ...
                    <p className="text-[10px] text-violet-600 font-bold uppercase tracking-widest">Espace transporteur</p>
                </div>
            </div>

            <div className="px-4 py-4 border-b border-slate-100 flex-shrink-0">
                <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl">
                    <div className="w-10 h-10 rounded-xl bg-violet-600 flex items-center justify-center text-white text-sm font-black flex-shrink-0">{initials}</div>
                    <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-slate-900 truncate">{prenom}</p>
                        <p className="text-xs text-slate-400">Transporteur PMR</p>
                    </div>
                </div>
            </div>
        ...
        <div className="min-h-screen bg-slate-50 flex">
            ...
            <div className="flex-1 lg:ml-64 flex flex-col min-h-screen">
                <header className="lg:hidden sticky top-0 z-20 bg-white border-b border-slate-100 h-14 flex items-center justify-between px-4">
                    ...
                    <p className="text-sm font-black text-slate-900">hs liberty · Transporteur</p>
                    <div className="w-9"/>
                </header>
...
function TruckIcon()  { return <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="3" width="15" height="13"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>; }
