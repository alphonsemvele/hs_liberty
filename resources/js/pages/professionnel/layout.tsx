...
export default function ProfessionnelLayout({ children, title, subtitle }: Props) {
    const { url, props } = usePage<any>();
    const [sidebarOpen, setSidebarOpen] = useState(false);

    const user     = props.auth?.user;
    const prenom   = user?.prenom ?? user?.name?.split(' ')[0] ?? 'Professionnel';
    const initials = ((user?.prenom?.[0] ?? '?') + (user?.nom?.[0] ?? '')).toUpperCase();

    const nav = [
        { href: '/professionnel',               label: 'Tableau de bord', icon: <HomeIcon />,   color: 'text-pink-600 bg-pink-50'    },
        { href: '/professionnel/interventions', label: 'Interventions',   icon: <HeartIcon />,  color: 'text-rose-600 bg-rose-50'    },
        { href: '/professionnel/planning',      label: 'Planning',        icon: <CalIcon />,    color: 'text-sky-600 bg-sky-50'      },
        { href: '/professionnel/profil',        label: 'Mon profil',      icon: <UserIcon />,   color: 'text-amber-600 bg-amber-50'  },
    ];

    const isActive = (path: string) => {
        const cur = url.split('?')[0].replace(/\/$/, '');
        const cln = path.replace(/\/$/, '');
        return cur === cln || (cln !== '/professionnel' && cur.startsWith(cln));
    };
        ...
        <>
            <div className="flex items-center gap-3 px-5 h-16 border-b border-slate-100 flex-shrink-0">
                ...
                <div>
                    ...
                    <p className="text-[10px] text-pink-600 font-bold uppercase tracking-widest">Espace pro santé</p>
                </div>
            </div>

            <div className="px-4 py-4 border-b border-slate-100 flex-shrink-0">
                <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl">
                    <div className="w-10 h-10 rounded-xl bg-pink-600 flex items-center justify-center text-white text-sm font-black flex-shrink-0">{initials}</div>
                    <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-slate-900 truncate">{prenom}</p>
                        <p className="text-xs text-slate-400">Professionnel de santé</p>
                    </div>
                </div>
            </div>
        ...
        <div className="min-h-screen bg-slate-50 flex">
            ...
            <div className="flex-1 lg:ml-64 flex flex-col min-h-screen">
                <header className="lg:hidden sticky top-0 z-20 bg-white border-b border-slate-100 h-14 flex items-center justify-between px-4">
                    ...
                    <p className="text-sm font-black text-slate-900">hs liberty · Pro</p>
                    <div className="w-9"/>
                </header>
...
function HeartIcon() { return <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>; }
